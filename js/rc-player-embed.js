...
function updateUrl(playing) {
	...
	if (window.bitrate != null) {
		if (playing)
			window.location = "/embed.html#p" + window.bitrate;
		else
			window.location = "/embed.html#" + window.bitrate;
	}
}
...
function selectBitrate(value, log = true) {
	...
	if (log) {
		if (window.playStatus)
			window.history.pushState(null, document.title + " " + value, "/embed.html#p" + value);
		else
			window.history.pushState(null, document.title + " " + value, "/embed.html#" + value);
	}
